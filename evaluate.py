...
DATA_DIRECTORY = './data/'
DATA_LIST_PATH = './data/test.lst'
NUM_CLASSES = 14
NUM_STEPS = 500  # Number of images in the validation set.
# INPUT_SIZE = '769,769'
# RESTORE_FROM = './deeplab_resnet.ckpt'
# RESTORE_FROM = 'snapshots/deeplabv3/CS_scenes_150000.pth'
# RESTORE_FROM = 'snapshots/deeplabv3/CS_scenes_150000.pth'
RESTORE_FROM = 'snapshots/color-new-da/CS_scenes_150000.pth'
...
def main():
    ...
    testloader = data.DataLoader(
        ModaDataset(args.data_dir, args.data_list, mean=IMG_MEAN, mirror=False, rotate=False),
        batch_size=1, shuffle=False, pin_memory=True)

    data_list = []
    confusion_matrix = np.zeros((args.num_classes, args.num_classes))
    palette = get_palette(14)
    interp = nn.Upsample(size=(600, 400), mode='bilinear', align_corners=True)

    if not os.path.exists('outputs/color-new-da'):
        os.makedirs('outputs/color-new-da')
    ...
    for index, batch in enumerate(testloader):
        ...
        output_im.save('outputs/color-new-da/' + name[0] + '.png')

...
class DPCModule(nn.Module):
    ...
    def forward(self, x):
        ...
        cpc_c1 = self.ca(cpc)
        cpc_c1_output = self.conv_red_1(cpc_c1)

        cpc_p = self.pa(cpc_c1)
        cpc_p_output = self.conv_red_2(cpc_p)

        cpc_c2 = self.ca(cpc_p)
        cpc_c2_output = self.conv_red_3(cpc_c2)

        out = self.bottlneck(torch.cat([x, cpc_c1_output, cpc_p_output, cpc_c2_output], 1))

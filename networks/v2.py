...
class DPCModule(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv_red_1 = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(512, out_channels, 1))
        self.conv_red_2 = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(512, out_channels, 1))
        self.conv_red_3 = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(512, out_channels, 1))
    ...
    def forward(self, x):
        ...
        pcp_p1 = self.pa(pcp)
        pcp_p1_output = self.conv_red_1(pcp_p1)

        pcp_c = self.ca(pcp_p1)
        pcp_c_output = self.conv_red_2(pcp_c)

        pcp_p2 = self.pa(pcp_c)
        pcp_p2_output = self.conv_red_3(pcp_p2)

        out = self.bottlneck(torch.cat([x, pcp_p1_output, pcp_c_output, pcp_p2_output], 1))

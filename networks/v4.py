...
class DPCModule(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.bottlneck = nn.Sequential(
            nn.Conv2d(inter_channels*3 + in_channels, out_channels, kernel_size=3, padding=1, dilation=1, bias=False),
    ...
    def forward(self, x):
        ...
        cpc_c1 = self.ca(cpc)
        cpc_c1_output = self.conv_red(cpc_c1)

        cpc_p = self.pa(cpc_c1)
        cpc_p_output = self.conv_red(cpc_p)

        cpc_c2 = self.ca(cpc_p)
        cpc_c2_output = self.conv_red(cpc_c2)

        # PCP
        pcp = self.conv1(x)

        pcp_p1 = self.pa(pcp)
        pcp_p1_output = self.conv_red(pcp_p1)

        pcp_c = self.ca(pcp_p1)
        pcp_c_output = self.conv_red(pcp_c)

        pcp_p2 = self.pa(pcp_c)
        pcp_p2_output = self.conv_red(pcp_p2)


        # fuse
        stage1 = cpc_c1_output + pcp_p1_output
        stage1_output = self.conv_red(stage1)

        stage2 = cpc_p_output + pcp_c_output
        stage2_output = self.conv_red(stage2)

        stage3 = cpc_c2_output + pcp_p2_output
        stage3_output = self.conv_red(stage3)

        out = self.bottlneck(torch.cat([x, stage1_output, stage2_output, stage3_output], 1))
